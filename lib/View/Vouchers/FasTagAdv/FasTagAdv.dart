...
class FasTagAdv extends StatefulWidget {
  const FasTagAdv({Key? key}) : super(key: key);

  @override
  State<FasTagAdv> createState() => _FasTagAdvState();
}

List<String> vehicleList2 = ['Select Vehicle',"10", "20", "30", "40"];
List<String> bpclAdvList = ['Select Type', 'BPCL', 'Cash', 'ATM'];

class _FasTagAdvState extends State<FasTagAdv> with Utility{
  ...
  // APi
  fastTagTransactionApiFunc(){
      setStateMounted(() {freshLoad = 1;});
    fastTagTransactionApi().then((value) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: UiDecoration.appBar('FastTag Advance'),
      body: SingleChildScrollView(
        child: Container(
          margin: const EdgeInsets.symmetric(vertical: 20,horizontal: 30),
          padding: const EdgeInsets.all(20),
          decoration: UiDecoration().formDecoration(color: const Color(0xffd6d6f5)),
          child:Form(
            key: _formKey,
            child: FocusScope(
  ...
  // API
  Future fastTagTransactionApi() async {
    var headers = {
      'token': Auth.token,
      'Content-Type': 'application/x-www-form-urlencoded'
    };
    var url = Uri.parse("${GlobalVariable.baseURL}Account/FastTagTransaction");
    var body = {
