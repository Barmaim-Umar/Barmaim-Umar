...
class AdvanceCashList extends StatefulWidget {
  const AdvanceCashList({Key? key}) : super(key: key);

  @override
  State<AdvanceCashList> createState() => _AdvanceCashListState();
}

List<String> assignDropdownList = ['All Vehicle', 'Not Assign', 'Assign'];
List<String> entriesDropdownList = ["10", "20", "30", "40"];

class _AdvanceCashListState extends State<AdvanceCashList> with Utility {
  ...
  advCashFetchApiFunc() {
    ...
    advCashFetchApi().then((value) {
      var info = jsonDecode(value);
      if (info['success'] == true) {
        cashTableList.clear();
        cashTableList.addAll(info['data']);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: UiDecoration.appBar("Advance Cash List"),
      body: Container(
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: TextDecorationClass().heading('Advance Cash List'),
            ),
            const Divider(),
            // dropdown & search
            Padding(
    ...
    return /* cashTableList.isEmpty ? const Center(child: Text("cashTableList.isEmpty \nUpdating List..."),) : */
        DataTable(
          ...
          DataColumn(label: Text('Action')),
            ...
            rows: List.generate(cashTableList.length, (index) {
              return DataRow(color: MaterialStatePropertyAll(index == 0 || index % 2 == 0 ? ThemeColors.tableRowColor : Colors.white), cells: [
                DataCell(Text(UiDecoration().getFormattedDate(cashTableList[index]["transaction_date"].toString()))),
                DataCell(Text(cashTableList[index]['vehicle_number'].toString() )),
                DataCell(Text(cashTableList[index]['user_name'].toString() )),
                const DataCell(Text('800')),
                DataCell(Container(
                  padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 3),
                  decoration: BoxDecoration(
                    color: cashTableList[index]['transaction_status'] == 1 ? ThemeColors.greenColor : Colors.orange,
                    borderRadius: BorderRadius.circular(3),
                  ),
                  child: Text(
                    cashTableList[index]['transaction_status'] == 1 ? 'Success' : 'Pending',
                ...
                DataCell(Row(
                  children: [
                    ...
                    Text(cashTableList[index]['remark'] ?? '_'),
                  ],
                )),
                DataCell(Text(cashTableList[index]['entry_by'].toString() )),
  ...
  // API
  Future advCashFetchApi() async {
    var headers = {'token': Auth.token};
    var url = Uri.parse(
        "${GlobalVariable.baseURL}Account/CashTransactionReport?limit=$entriesDropdownValue&page=${GlobalVariable.currentPage}&keyword=${searchController.text}&from_date=${fromDateApi.text}&to_date=${toDateApi.text}");
