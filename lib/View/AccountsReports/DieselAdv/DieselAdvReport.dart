...
class DieselAdvReport extends StatefulWidget {
  const DieselAdvReport({Key? key}) : super(key: key);

  @override
  State<DieselAdvReport> createState() => _DieselAdvReportState();
}

List<String> assignDropdownList = ['All Vehicle' , 'Not Assign' , 'Assign'];
List<String> entriesDropdownList = ["10" ,"20" ,"30" ,"40"];

class _DieselAdvReportState extends State<DieselAdvReport> with Utility{
  ...
  dieselFetchApiFunc(){
    ...
    dieselFetchApi().then((value) {
      var info = jsonDecode(value);
      if(info['success'] == true){
        dieselTableList.clear();
        dieselTableList.addAll(info['data']);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: UiDecoration.appBar("Diesel Adv Report"),
      body: Container(
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: TextDecorationClass().heading('Diesel Adv Report'),
            ),
            const Divider(),
            // dropdown & search
            Padding(
            ...
            Container(
              ...
              child: Row(
                ...
                children: const [
                  Text("Doc Expiry Report" , style: TextStyle(color: Colors.white , fontWeight: FontWeight.bold),),
                ],),
            ),
            Expanded(
  ...
  Widget buildDataTable() {
    return /* dieselTableList.isEmpty ? const Center(child: Text("ledgerTableList.isEmpty \nUpdating List..."),) : */
      DataTable(
          columns: const [
            DataColumn(label: Text('Date')),
            DataColumn(label: Text('Vehicle Number')),
          ...
          rows: List.generate(dieselTableList.length, (index) {
            return DataRow(
                color:  MaterialStatePropertyAll(index == 0 || index % 2 == 0? ThemeColors.tableRowColor : Colors.white ),
                cells: [
                  DataCell(Text(UiDecoration().getFormattedDate(dieselTableList[index]['transaction_date'].toString()))),
                  DataCell(Text(dieselTableList[index]['vehicle_id'].toString())),
                  DataCell(Text(dieselTableList[index]['driver_name'].toString())),
                  DataCell(Text(dieselTableList[index]['liters'].toString())),
                  DataCell(Text(dieselTableList[index]['desile_rate'].toString())),
                  DataCell(Text(dieselTableList[index]['total_amount'].toString())),
                  DataCell(Container(
                    padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 3),
                    decoration: BoxDecoration(
                      color: dieselTableList[index]['transaction_status'] == 1 ? ThemeColors.greenColor : Colors.orange,
                      borderRadius: BorderRadius.circular(3),
                    ),
                    child: Text(
                      dieselTableList[index]['transaction_status'] == 1 ? 'Success' : 'Pending',
                  ...
                  DataCell(Row(
                    children: [
                      ...
                      Text(dieselTableList[index]['remark'].toString()),

                    ],
                  )),
                  DataCell(Text(dieselTableList[index]['user_name'].toString())),
                  DataCell(Row(
                    children: [
                     // delete Icon
                      Container(
  ...
  // API
  Future dieselFetchApi() async {
    var headers = {
      'token': Auth.token
    };
    var url = Uri.parse("${GlobalVariable.baseURL}Account/DieselTransactionReport?limit=$entriesDropdownValue&page=${GlobalVariable.currentPage}&keyword=${searchController.text}&from_date=${fromDateApi.text}&to_date=${toDateApi.text}");
